...
plt.legend()
# plt.show()
plt.savefig('figures/illustration_loss.jpg')
...
with plt.rc_context(bundles.jmlr2001(ncols=4, rel_width=1.777)):
    ...
    axs[0].fill_between(x.squeeze(), m_map - s_map, m_map + s_map, color='tab:red', alpha=0.3)
    # Bayes homoscedastic
    ...
    axs[1].fill_between(x.squeeze(), m_bayes - s_emp, m_bayes - s_bayes, color='tab:red', alpha=0.3)
    axs[1].fill_between(x.squeeze(), m_bayes + s_emp, m_bayes + s_bayes, color='tab:red', alpha=0.3)
    # MAP heteroscedastic
    axs[2].plot(x, mh_map, color='black')
    axs[2].set_title('Heteroscedastic MAP')
    axs[2].fill_between(x.squeeze(), mh_map - sh_map, mh_map + sh_map, color='tab:red', alpha=0.3)
    ...
    axs[3].fill_between(x.squeeze(), mh_bayes - sh_emp, mh_bayes - sh_bayes, color='tab:red', alpha=0.3, label='aleatoric')
    axs[3].fill_between(x.squeeze(), mh_bayes + sh_emp, mh_bayes + sh_bayes, color='tab:red', alpha=0.3)
